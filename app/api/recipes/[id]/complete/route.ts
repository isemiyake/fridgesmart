...
export async function POST(
  ...
  for (const ri of recipe.ingredients) {
    ...
    usedCo2.push(ri.normalized.co2SavedGrams);

    const reqInItemUnit = convert(ri.quantity, ri.unit, item.unit);
    if (reqInItemUnit === null) {
      // unités non comparables -> on considère l'ingrédient consommé
  ...
  await prisma.cookedHistory.create({
    data: {
      ...
      recipeSnapshot: JSON.stringify({
        title: recipe.title,
        ingredients: recipe.ingredients.map((ri) => ri.normalized.name),
      }),
      co2Saved,
      xpEarned,
    },
